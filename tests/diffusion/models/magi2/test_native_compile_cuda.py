...
pytestmark = [
    ...
    *hardware_marks(res={"cuda": "L4"}, num_cards=1),
    # ROCm also exposes torch.cuda, but this path uses vLLM CUDA FlashAttention.
    pytest.mark.skipif(
        not current_omni_platform.is_cuda() or not torch.cuda.is_available(),
        reason="requires vLLM CUDA FlashAttention",
    ),
]
