...
The WSL CPU and CPU+Radeon InternVLA cells also have [bounded Omni complete-policy profiles](../../../e2e_expansion_20260923/evidence/internvla_omni_policy/README.md): one warmup and 20 serial patterned synthetic requests per placement, plus admission refusal, cancellation and a public hybrid `AsyncOmni.generate` request. A separate [native Windows CPU Omni profile](../../../e2e_expansion_20260923/evidence/internvla_omni_windows_cpu/README.md) adds 20 serial patterned requests, refusal, cancellation and a public action request. These advance only the synthetic action-policy path. Real observations/reference actions, physical action units/order/step time, task quality, loading peak and sustained power/thermal behavior remain unverified. The earlier [Cosmos component and direct hybrid results](../../../e2e_expansion_20260923/evidence/internvla_cosmos_radeon890m/README.md) remain separate experiments.
...
| PC HX370 + Radeon 890M / Windows worker | scoped E2E: Omni Spark text | scoped E2E: Omni Radeon talker/codec + CPU predictor | NOT E2E | scoped E2E: Omni Radeon+CPU image+audio → text+speech | synthetic policy pass: Omni CPU policy + Radeon Cosmos |
...
| PC HX370 + Radeon 890M / Windows worker | InternVLA-A1 | SYNTHETIC_POLICY_PASS_HYBRID_OMNI; NOT_TASK_E2E | The real Place_Markpen checkpoint ran in an admitted Omni whole-policy stage: BF16 Qwen3-VL/flow action policy on WSL CPU and the pinned six-frame FP32 Cosmos encoder in Omni's external DirectML worker, with selected Radeon 890M and output-device placement verified. One warmup plus 20 serial patterned synthetic camera/state/noise requests returned identical finite `[1,50,32]` action chunks; nearest-rank complete-request wall p50/p95 was 3.120/3.197 s versus a separately run Omni CPU baseline of 3.175/3.219 s. The hybrid startup was 43.36 s versus CPU 40.03 s. Hybrid-vs-CPU action relative L2 difference was 2.305%, cosine 0.999737; no task tolerance exists. The FP32 encoder is an explicit precision change after DirectML BF16 aborted. The stage verified hashes, reserved 16 GiB shared host RAM, refused an insufficient reservation on the CPU route, emitted a terminal action event through public `AsyncOmni.generate`, and released its reservation after in-flight cancellation and shutdown. An initial hybrid shutdown quarantined 16 GiB; a lifecycle fix and reruns cleared it. This is output-device evidence, not internal per-op attribution or robot-task quality. The older direct hybrid's zero-input 2.952/3.322 s profile is a distinct fixture/runtime and cannot be compared as the same run. | Validate real observation/reference-action quality, physical units/order/step time, post-cancel restart, loading peak, concurrent admission and sustained tail/power before deployment; [Omni raw evidence](../../../e2e_expansion_20260923/evidence/internvla_omni_policy/README.md), [earlier direct hybrid/component evidence](../../../e2e_expansion_20260923/evidence/internvla_cosmos_radeon890m/README.md). |
