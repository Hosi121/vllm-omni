...
| PC HX370 + Radeon 890M / Windows worker | scoped E2E: Omni Spark text | scoped E2E: Omni Radeon talker/codec + CPU predictor | NOT E2E | NOT E2E | NOT E2E |
...
| PC HX370: CPU / native Windows | MiniCPM-o 4.5 | SCOPED_E2E_STANDALONE_COMPLETE_PATH | The pinned Q4_K_M language and F16 vision/audio/TTS/Token2Wav GGUF set completed **one combined synthetic image+tone → text+speech** request through a native Windows x86-64-v3 CPU C++ CLI. The verifier found actual index-1 image and audio prefill, nonempty text, a completion marker, and 50 contiguous nonzero 24 kHz WAV chunks (49.16 s audio). One cold process took 137.34 s; sampled RSS/private-byte maxima were 13.10/13.31 GB. The first generic cross-build exited 0 after 615.19 s with only 15 chunks and no completion marker; the CLI wait was patched to be bounded and to fail on timeout. The final text was not grounded in the red square/tone, and the test supplied no explicit input-description task; semantic quality is unqualified. This is a standalone CPU route, not native vLLM or an Omni StageRuntime. The optional upstream WAV merger still uses the wrong chunk names; the verifier assembled original PCM after completion. | Bind a full-session native CPU stage with admission and state/cancel semantics; validate prompted image/audio quality, speech intelligibility, streaming, loading peak, repeated requests and sustained thermal behavior. [Complete raw evidence](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_windows_cpu/README.md), [failed first run](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_windows_cpu_attempt1/README.md). |
