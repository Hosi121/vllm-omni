...
All 60 pairings have a disposition. The timing tables below remain the 2026-09-22 measurements. The configuration matrix and affected per-cell reasons incorporate the [2026-09-23 scoped recovery runs](../../../e2e_recovery_20260923/README.md) and [WSL CPU/native Windows expansion](../../../e2e_expansion_20260923/README.md). The expansion adds separate 20-request serial MiniCPM-o text-to-speech profiles on WSL CPU and native Windows GPU; other new paths are single-request or short-stream runs. Profiling completion does not imply full release qualification.
...
|---|---|---|---|---|---|
| PC HX370: CPU / WSL | profile completed | scoped E2E: short stream | NOT E2E | scoped E2E: text → speech | synthetic policy pass |
| PC HX370: CPU / native Windows | NOT E2E | NOT E2E | NOT E2E | NOT E2E | synthetic policy pass |
| PC HX370 + RTX 5090 Laptop / WSL | profile completed | profile completed | scoped E2E: text + image | scoped E2E: text/image → speech | synthetic policy pass |
| PC HX370 + RTX 5090 Laptop / Windows | profile completed | profile completed | NOT E2E | scoped E2E: text → speech | synthetic policy pass |
...
| PC HX370: CPU / WSL | MiniCPM-o 4.5 | SCOPED_E2E | The verified checkpoint completed real three-stage text-to-text+speech requests on the vLLM 0.28 CPU backend: coherent text and nonzero 5.2 s WAVs. A separate 1-warmup/20-measured serial run had request wall p50 21.86 s/p95 22.97 s, with one consistent thinker token sequence. WSL swap reached 5.75 GB; speech meaning was not checked. | Validate speech intelligibility, other input modalities, concurrency, interruption, admission/loading peak and sustained thermal behavior before broader CPU qualification; [CPU evidence](../../../e2e_expansion_20260923/README.md). |
...
| PC HX370 + RTX 5090 Laptop / WSL | MiniCPM-o 4.5 | SCOPED_E2E | The verified checkpoint completed three-stage text-to-text+speech under a constrained 24 GiB profile with 5 GiB host offload. A separate one-image plan described a synthetic 448×448 red square on white correctly and produced a nonzero 5.2 s WAV in a 17.49 s request. The original 0.58 thinker GPU budget failed image/KV admission (0.10 GiB available versus 0.28 GiB required); 0.61 admitted the same context and image. Stage 0 needed a force kill at shutdown, and speech meaning was not checked. | Validate broader images, audio/video input, speech intelligibility, interruption, repeated latency, shutdown cleanup and sustained power; [text evidence](../../../e2e_recovery_20260923/README.md), [image evidence](../../../e2e_expansion_20260923/README.md). |
...
| PC HX370 + RTX 5090 Laptop / Windows | MiniCPM-o 4.5 | SCOPED_E2E | The verified checkpoint completed native Windows three-stage BF16 text-to-text+speech requests with 5 GiB thinker host offload and explicit 2,048/512-token limits. Text and all 32 thinker token IDs matched WSL CUDA; a 5.2 s WAV was nonzero. After a 300 s default startup timeout on the first attempt, a 900/600 s limit allowed a functional pass. A separate 1-warmup/20-measured serial run had request wall p50 18.38 s/p95 18.73 s; sampled host available RAM fell to 0.78 GB and pagefile use reached 11.17 GB. Speech meaning remains unverified. | Check speech intelligibility/alignment, other input modalities, concurrency, interruption, loading-peak admission and sustained thermal/power behavior before broader qualification; [native evidence](../../../e2e_expansion_20260923/README.md). |
