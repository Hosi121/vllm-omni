...
All 60 pairings have a disposition. The timing tables below remain the 2026-09-22 measurements. The configuration matrix and affected per-cell reasons incorporate the [2026-09-23 scoped recovery runs](../../../e2e_recovery_20260923/README.md); those single-request or short-stream runs are not additional completed profiling protocols. Profiling completion does not imply full release qualification.

## Configuration × model

NOT E2E means no complete device/model workload was verified. “Scoped E2E” identifies only the named input/output path; “synthetic policy pass” does not establish a real robot-task result. See the per-cell reasons below.

| Device | Spark-X2.5 | Qwen3-TTS 0.6B CustomVoice | Qwen3.8-27B | MiniCPM-o 4.5 | InternVLA-A1 |
|---|---|---|---|---|---|
| PC HX370: CPU / WSL | profile completed | scoped E2E: short stream | NOT E2E | NOT E2E | NOT E2E |
| PC HX370: CPU / native Windows | NOT E2E | NOT E2E | NOT E2E | NOT E2E | NOT E2E |
| PC HX370 + RTX 5090 Laptop / WSL | profile completed | profile completed | scoped E2E: text + image | scoped E2E: text → speech | synthetic policy pass |
...
| PC HX370: CPU / WSL | MiniCPM-o 4.5 | NOT_E2E_PROFILEABLE | The exact MiniCPM-o 4.5 checkpoint is now downloaded and SHA-256 verified, but no CPU memory admission or complete CPU pipeline has been run. The RTX result does not establish CPU support. | Validate a CPU execution plan and complete text/speech path under its actual memory budget; [weight evidence](../../../e2e_recovery_20260923/evidence/weights/minicpmo.json). |
| PC HX370: CPU / WSL | InternVLA-A1 | NOT_E2E_PROFILEABLE | Base and Place_Markpen checkpoints are now SHA-256 verified, but the policy has only run on CUDA with synthetic inputs. No CPU policy forward or task-quality result exists. | Admit the full CPU model and run the policy on real observations with reference actions; [recovery evidence](../../../e2e_recovery_20260923/README.md). |
...
| PC HX370: CPU / native Windows | InternVLA-A1 | NOT_E2E_PROFILEABLE | Base and fine-tuned checkpoints are verified in the local Linux model store. No native Windows policy run, model access or qualified CPU backend exists; the installed vLLM CPU attention operators are incomplete. | Validate a native policy backend, real observations and reference actions; [recovery evidence](../../../e2e_recovery_20260923/README.md). |
...
| PC HX370 + RTX 5090 Laptop / WSL | MiniCPM-o 4.5 | SCOPED_E2E | The verified real checkpoint completed one three-stage text-to-text+speech request under a constrained 24 GiB profile with 5 GiB host offload. Text was coherent and a nonzero 5.2 s WAV was produced in a single 20.53 s request; speech meaning was not checked. | Validate speech intelligibility/alignment, image and audio input, interruption, sustained load and power; [raw log](../../../e2e_recovery_20260923/evidence/minicpmo/mini_fixed_e2e.log). |
...
| PC HX370 + RTX 5090 Laptop / Windows | MiniCPM-o 4.5 | NOT_E2E_PROFILEABLE | The exact checkpoint is verified and one constrained three-stage text/speech request passed in WSL; no native Windows execution, memory admission or speech-quality check exists. | Repeat the real three-stage pipeline on native Windows and verify speech and sustained behavior; [WSL-only evidence](../../../e2e_recovery_20260923/README.md). |
| PC HX370 + RTX 5090 Laptop / Windows | InternVLA-A1 | NOT_E2E_PROFILEABLE | Base and fine-tuned checkpoints are verified, but the only policy forward used synthetic observations under WSL/CUDA; no native Windows run or real-task qualification exists. | Run a native Windows policy forward, then compare real observations against reference actions; [WSL-only evidence](../../../e2e_recovery_20260923/README.md). |
