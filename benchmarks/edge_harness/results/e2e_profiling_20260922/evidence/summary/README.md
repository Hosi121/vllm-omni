...
| PC HX370: CPU / native Windows | scoped E2E: Omni Spark text | scoped E2E: Omni CPU text → complete WAV | NOT E2E | scoped E2E: standalone GGUF image+audio → text+speech | synthetic policy pass |
...
| PC HX370 + Radeon 890M / Windows worker | scoped E2E: Omni Spark text | scoped E2E: Omni Radeon talker/codec + CPU predictor | NOT E2E | scoped E2E: standalone Radeon+CPU image+audio → text+speech | NOT E2E |
...
| PC HX370: CPU / WSL | MiniCPM-o 4.5 | SCOPED_E2E | The verified BF16 checkpoint completed real three-stage text, one synthetic image and one synthetic audio input to text+speech on the vLLM 0.28 CPU backend in separate bounded plans. The 448×448 red square was described correctly with a nonzero 5.2 s WAV in one 37.70 s request; the 2 s/440 Hz tone was described as “A loud and high pitched beep” with a nonzero 2.16 s WAV in one 17.30 s request. CPU wording/token IDs differ from the WSL GPU runs, with no task-quality threshold established. A separate 1-warmup/20-measured serial **text** run had request wall p50 21.86 s/p95 22.97 s. WSL swap reached 5.75 GB during that text profile; manual snapshots in the multimodal runs showed heavy swap use but do not prove peak memory. Both multimodal runs force-killed stage 0 at shutdown and reported shared-memory cleanup warnings. Separately, a pinned Q4_K_M/F16 GGUF set ran one **combined** synthetic image+tone input to text and 9.24 s of speech through standalone C++ on WSL CPU in 40.70 s, with 13,369,604 KiB (~12.75 GiB) maximum RSS; its generic response did not establish input-grounded quality, and its optional WAV merger used the wrong chunk filename. The actual ten WAV chunks were verified and assembled without changing PCM. | Validate real image/audio suites, speech intelligibility, video input, concurrency, interruption, admission/loading peak, clean shutdown and sustained thermal behavior; check the C++ GGUF route's semantic quality and Omni binding separately. [vLLM CPU evidence](../../../e2e_expansion_20260923/README.md), [C++ CPU raw evidence](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_cpu/README.md). |
...
| PC HX370: CPU / native Windows | MiniCPM-o 4.5 | SCOPED_E2E_STANDALONE_COMPLETE_PATH | The pinned Q4_K_M language and F16 vision/audio/TTS/Token2Wav GGUF set completed a combined synthetic JPEG plus spoken “What color is the square in the image?” input to text and nonzero speech on native Windows CPU. The correct answer, “The square in the image is red.”, used the same eight language token IDs as the Radeon hybrid; independent Whisper tiny.en ASR transcribed both the input and 2.32 s output with WER 0. One cold prompted request took 24.66 s, with 13.11 GB maximum sampled RSS. An earlier unprompted square+tone request produced generic text and 49.16 s of speech in 137.34 s; the first generic build exited 0 with incomplete audio and is preserved as a failure. This is a standalone C++ route, not native vLLM or Omni; one synthetic quality case is not general qualification. The optional upstream WAV merger still has a filename/timing bug, so verified chunks were assembled after completion. | Bind a whole-session native CPU Omni stage with admission/state/cancellation; validate real image/audio suites, playable streaming, loading peak, repeated latency and sustained thermal behavior. [Prompted CPU evidence](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_windows_cpu_prompted/README.md), [unprompted evidence](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_windows_cpu/README.md), [failed first run](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_windows_cpu_attempt1/README.md). |
...
| PC HX370 + Radeon 890M / Windows worker | MiniCPM-o 4.5 | SCOPED_E2E_STANDALONE_COMPLETE_PATH_HYBRID | The pinned Q4_K_M/F16 GGUF set completed a **combined spoken question+image → text+speech** path through standalone C++ on native Windows. `GGML_VK_VISIBLE_DEVICES=1` exposed only Radeon 890M; logs show all 37 language layers and vision on Vulkan0, TTS weights at 0/21 GPU layers, and CPU Token2Wav/vocoder. The spoken “What color is the square in the image?” was independently transcribed with WER 0; the model answered “The square in the image is red.” with the same eight language token IDs as CPU, and the 2.56 s WAV was transcribed with WER 0. One cold prompted process took 18.63 s, sampled RSS 15.57 GB. A separate unprompted square+tone run completed 42 WAV chunks/41.96 s audio in 113.43 s. This is CPU+iGPU joint execution, not all-Radeon, NPU or an Omni stage; one synthetic quality case and one cold time are not release qualification. | Bind a full-session Omni stage with explicit shared-RAM/GPU admission and cancellation; validate real multimodal quality, playable streaming, loading peak, repeated/concurrent latency and sustained thermal behavior. [Prompted hybrid evidence](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_radeon890m_prompted/README.md), [unprompted placement and raw run](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_radeon890m/README.md), [CPU parity](../../../e2e_expansion_20260923/evidence/minicpmo_cpp_windows_cpu_prompted/README.md). |
