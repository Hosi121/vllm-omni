...
| CPU-only control, run later | One **73.63 s** request with the same fixture | 51.80 s | Same 21 GiB budget but **32.28 GB** OS-available before load; sampled worker RSS 13.10 GB ([report](cpu_control_current.json), [worker log](cpu_control_worker.log)) |
...
These are different runs, not a paired speed comparison. The new repeated Radeon profile and cold rerun both passed functionally but were roughly five times slower than the older single cold request. Each of the 21 successful new Radeon worker logs contains two `PrefetchVirtualMemory failed` warnings, while the earlier single-request log contains none. Representative new Radeon language-model load times were about 25–27 s versus 6.07 s in the earlier log. The later CPU-only control used the same model/input hashes and pinned CPU CLI SHA256 `29247187ddf3b13591cd14cc070e5e25c6bb7547296acc5e8a74e2945811d725`. It also reported two prefetch warnings and a 36.03 s model load, versus 13.53 s in its earlier 24.34 s cold Omni smoke. Its complete request took 73.63 s despite more OS-available RAM at admission. The CPU control answered correctly and emitted a [1.96 s WAV](cpu_control_current.wav); pinned Whisper tiny.en transcribed “The square and the image is red.” (WER 0.143 for one output, [ASR report](cpu_control_current_asr.json)). These observations show the slowdown is not unique to Radeon or explained by **pre-load** free RAM alone. Memory pressure, changed UNC file-cache/paging conditions, and power/thermal state remain possible explanations, **not established causes**. Full OS commit/pagefile time series, shared-iGPU allocation and a truly matched-condition CPU/Radeon sequence were not captured. Do not reuse the older 18.27 s result as expected warmed performance or claim the Radeon path is faster than CPU under this condition.
