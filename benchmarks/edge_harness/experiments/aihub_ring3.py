...
mdl = hub.upload_model(f"{HERE}/onnx/ring3_sliding.onnx", name="spark25_ring3_sliding")
pend, res = {}, {}
for q, o in OPT.items():
    cj = hub.submit_compile_job(model=mdl, device=hub.Device(DEV),
                                name=f"spark ring3 sliding {q}", options=o)
...
for q, pj in prof.items():
    ...
    if st.success:
        ...
        json.dump(p, open(f"{HERE}/profile_ring3_sliding_{q}.json","w"))
        print(f"  ring3 sliding {q}: {res[q]:.3f} ms", flush=True)
    else:
        res[q] = None; print(f"  {q} FAILED:", (st.message or "")[:200], flush=True)
    json.dump(res, open(f"{HERE}/aihub_ring3.json","w"), indent=1)
