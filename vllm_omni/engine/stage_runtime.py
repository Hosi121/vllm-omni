...
class StageRuntime:
    ...
    def _initialize_local_llm_replica(
        ...
        try:
            ...
            #
            # [edge-infer] the fork's init profiler (init_timeline / init_profile)
            # consumes the phase names emitted here, so the single upstream path
            # keeps emitting all four: g2_launch_lock_wait, g2_spawn_and_ready,
            # and within the latter g2_spawn (under the lock) and g2_ready_wait
            # (lock released). The READY wait is the ExitStack close.
            ...
            with (
                worker_phase("g2_spawn_and_ready", stage_id=plan.metadata.stage_id, replica_id=plan.replica_id),
                ExitStack() as launch_stack,
            ):
                try:
                    with (
                        worker_phase("g2_spawn", stage_id=plan.metadata.stage_id, replica_id=plan.replica_id),
