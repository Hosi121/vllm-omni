...
def run_headless(args: TrackingNamespace) -> None:
    ...
    # ``--deploy-profile`` (edge) translates to a concrete ``deploy_config`` path
    # and applies the profile's ``orchestrator:`` defaults, exactly like the
    # standard engine path (AsyncOmniEngine.__init__), so headless and standard
    # launches resolve to the same per-stage device layout. Must run before
    # deploy_config is popped below.
